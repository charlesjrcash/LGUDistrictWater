...
export default function WaterRatesPage() {
  ...
  const [customerClassifications, setCustomerClassifications] =
  useState<CustomerClassification[]>([]);

const [meterSizesMaster, setMeterSizesMaster] =
  useState<MeterSize[]>([]);
...
async function loadWaterRateOptions() {
  try {
    ...
    const response = await fetch(
      "/api/water-rate-options"
    );

    if (!response.ok) {
      throw new Error(
        "Failed to load classifications and meter sizes."
      );
    }
    ...
    if (!result.success) {
      throw new Error(
        result.message ||
          "Failed to load water rate options."
      );
    }
    ...
    console.error(
      "Water rate options loading failed:",
      error
    );
...
async function handleSaveRate(
  ...
  const [classificationFilter, setClassificationFilter] =
    useState("all");
  const [meterSizeFilter, setMeterSizeFilter] =
    useState("all");
  ...
  async function loadWaterRates() {
    try {
      ...
      if (!result.success) {
        throw new Error(
          result.message || "Failed to load water rates."
        );
      }
  ...
  const filteredRates = useMemo(() => {
    ...
    return rates.filter((rate) => {
      ...
      const matchesMeterSize =
        meterSizeFilter === "all" ||
        rate.meter_size === meterSizeFilter;
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage water consumption rates and
              effective periods.
            </p>
          </div>
