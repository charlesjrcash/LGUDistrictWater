...
type BillingRow = { customer_no: string; control_no: string; connection_status: string | null; meter_no: string | null; bill_no: string | null; bill_date: Date | null; due_date: Date | null; total_amount_due: string | null; bill_status: string | null; period_name: string | null; period_code: string | null; previous_reading: string | null; present_reading: string | null; consumption: string | null };
...
export async function POST(request: Request) {
  try {
    ...
    const result = await pool.query<BillingRow>(`
      SELECT customer.customer_no, sa.control_no, cs.status_name AS connection_status, meter.meter_no,
        ...
        JOIN service_accounts sa ON sa.customer_id = customer.customer_id
       ...
       WHERE UPPER(customer.customer_no) = UPPER($1)
          OR UPPER(sa.control_no) = UPPER($1)
       ORDER BY bill.bill_date DESC NULLS LAST, sa.service_account_id DESC
       LIMIT 1
    `, [accountNumber]);
    const account = result.rows[0];
    if (!account) return Response.json({ message: "No account was found with that number." }, { status: 404 });
    return Response.json({ customerNo: account.customer_no, accountNo: account.control_no, serviceStatus: account.connection_status || "Not available", meterNo: maskMeterNumber(account.meter_no), bill: account.bill_no ? { billNo: account.bill_no, billDate: account.bill_date, dueDate: account.due_date, amountDue: account.total_amount_due || "0", status: account.bill_status || "UNKNOWN", period: account.period_name || account.period_code || "Not available", previousReading: account.previous_reading, presentReading: account.present_reading, consumption: account.consumption } : null });
  } catch (error) {
