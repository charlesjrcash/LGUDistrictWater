...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({
      ...
      accountNo: account.control_no,
      serviceStatus: account.connection_status || "Not available",
      meterNo: maskMeterNumber(account.meter_no),
      bill: account.bill_no
        ? {
            billNo: account.bill_no,
            billDate: account.bill_date,
            dueDate: account.due_date,
            amountDue: account.total_amount_due || "0",
            status: account.bill_status || "UNKNOWN",
            period:
              account.period_name || account.period_code || "Not available",
            previousReading: account.previous_reading,
            presentReading: account.present_reading,
    ...
    return Response.json(
      { message: "Billing inquiry is temporarily unavailable." },
      { status: 500 },
    );
  }
}
