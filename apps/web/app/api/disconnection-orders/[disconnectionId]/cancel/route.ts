...
export async function POST(_: Request, { params }: Context) {
  ...
  try {
    const result = await db.query(`UPDATE disconnection_orders SET status='CANCELLED',updated_at=CURRENT_TIMESTAMP WHERE disconnection_id=$1 AND status='PENDING' RETURNING disconnection_id::text AS "disconnectionId"`, [orderId]);
