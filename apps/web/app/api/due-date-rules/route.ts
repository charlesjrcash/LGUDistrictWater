...
function parseDueDateRule(body: Record<string, unknown>) {
  ...
  if (
    dueDateRule.daysAfterBilling !== null &&
    (!Number.isInteger(dueDateRule.daysAfterBilling) ||
      dueDateRule.daysAfterBilling < 0)
  ) {
    return {
      error:
        "Days after billing must be a whole number greater than or equal to zero.",
    };
  }

  if (
    dueDateRule.fixedDay !== null &&
    (!Number.isInteger(dueDateRule.fixedDay) ||
      dueDateRule.fixedDay < 1 ||
...
export async function PUT(request: Request) {
  try {
    ...
    if (!/^\d+$/.test(ruleId)) {
      return Response.json(
        { success: false, message: "Due date rule ID is required." },
        { status: 400 },
      );
    }
    if ("error" in parsed) {
