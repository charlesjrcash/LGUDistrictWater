...
import {
  findInitialAccountStatus,
  nextControlNumber,
} from "@/modules/service-accounts/server";
  ...
  try {
    ...
    if (existing.rows[0])
      throw Object.assign(new Error("ALREADY_EXISTS"), {
        controlNo: existing.rows[0].control_no,
      });
    ...
    return Response.json(
      {
        success: true,
        data: { controlNo },
        message: `Service account ${controlNo} was created successfully.`,
      },
      { status: 201 },
    );
  } catch (error) {
    ...
    if (code === "APPLICATION_NOT_FOUND")
      return Response.json(
        {
          success: false,
          message: "The service application no longer exists.",
        },
        { status: 404 },
      );
    if (code === "NOT_APPROVED")
      return Response.json(
        {
          success: false,
          message: "Only approved applications can create a service account.",
        },
        { status: 409 },
      );
    if (
      ...
      return Response.json(
        {
          success: false,
          message: "This application already has a service account.",
        },
        { status: 409 },
      );
      ...
      return Response.json(
        {
          success: false,
          message:
            "A Pending Installation connection status must be configured first.",
        },
        { status: 409 },
      );
