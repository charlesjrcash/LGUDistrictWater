...
function creationPayload(body: Record<string, unknown>) {
  ...
  if (!payload.applicationNo)
    errors.applicationNo = "An approved application is required.";
  if (!payload.classificationCode)
    errors.classificationCode = "Select a classification.";
  if (!payload.connectionTypeCode)
    errors.connectionTypeCode = "Select a connection type.";
  if (payload.dateConnected && !isIsoDate(payload.dateConnected))
    errors.dateConnected = "Enter a valid connection date.";
...
function creationFailure(error: unknown) {
  ...
  if (code === "APPLICATION_NOT_FOUND")
    return ["Service application not found.", 404] as const;
  if (code === "NOT_APPROVED")
    return [
      "Only approved service applications can create a service account.",
      409,
    ] as const;
  if (code === "APPLICATION_ALREADY_USED")
    return [
      "Service account already exists for this application.",
      409,
    ] as const;
  if (code === "INVALID_CLASSIFICATION")
    return ["The selected classification is unavailable.", 400] as const;
  if (code === "INVALID_CONNECTION_TYPE")
    return ["The selected connection type is unavailable.", 400] as const;
  if (code === "INVALID_SERVICE_TYPE")
    return ["The selected service type is unavailable.", 400] as const;
  if (code === "INVALID_ROUTE")
    return ["The selected reading route is unavailable.", 400] as const;
  if (code === "INITIAL_STATUS_NOT_FOUND")
    return [
      "A pending-installation connection status must be configured before creating a service account.",
      409,
    ] as const;
  if (
    typeof error === "object" &&
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const summary = await db.query<{
      ...
      `SELECT COUNT(*)::text AS total, COUNT(*) FILTER (WHERE UPPER(COALESCE(cs.status_code,'')) = 'ACTIVE')::text AS active, COUNT(*) FILTER (WHERE UPPER(COALESCE(cs.status_code,'')) LIKE '%PENDING%')::text AS pending, COUNT(*) FILTER (WHERE UPPER(COALESCE(cs.status_code,'')) LIKE '%DISCONNECT%')::text AS disconnected FROM service_accounts acc LEFT JOIN mt_connection_status cs ON cs.connection_status_id = acc.connection_status_id`,
    );
    ...
    return Response.json(
      { success: false, message: "Unable to load service accounts." },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  if (Object.keys(errors).length)
    return Response.json(
      {
        success: false,
        message: "Please complete all required service account information.",
        errors,
      },
      { status: 400 },
    );
  ...
  try {
    ...
    const created = await createAccountFromApplication(client, {
      ...payload,
    ...
    return Response.json(
      {
        success: true,
        data: created,
        message: `Service account ${created.controlNo} was created successfully.`,
      },
      { status: 201 },
    );
  } catch (error) {
    ...
    if (failure)
      return Response.json(
        { success: false, message: failure[0] },
        { status: failure[1] },
      );
    console.error("Unable to create service account:", error);
    return Response.json(
      { success: false, message: "Unable to create the service account." },
      { status: 500 },
    );
  } finally {
    client.release();
