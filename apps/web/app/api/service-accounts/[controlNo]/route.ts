...
export async function GET(_request: Request, context: Context) {
  ...
  try {
    ...
    if (!result.rows[0]) return Response.json({ success: false, message: "Service account not found." }, { status: 404 });
    return Response.json({ success: true, data: result.rows[0] });
  } catch (error) {
    console.error("Unable to load service account:", error);
    return Response.json({ success: false, message: "Unable to load the service account." }, { status: 500 });
  }
}
...
export async function PUT(request: Request, context: Context) {
  ...
  try {
    ...
    const current = await client.query<{ service_account_id: string }>("SELECT service_account_id FROM service_accounts WHERE control_no = $1 FOR UPDATE", [controlNo]);
    ...
    if (code === "NOT_FOUND") return Response.json({ success: false, message: "Service account not found." }, { status: 404 });
    ...
    return Response.json({ success: false, message: "Unable to update the service account." }, { status: 500 });
