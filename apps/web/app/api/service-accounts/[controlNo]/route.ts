...
export async function PUT(request: Request, context: Context) {
  ...
  if (dateConnected && !isIsoDate(dateConnected))
  ...
  try {
    ...
    const current = await client.query<{
      ...
      connection_status_id: string | null;
    }>(
      "SELECT service_account_id, connection_status_id FROM service_accounts WHERE control_no = $1 FOR UPDATE",
      [controlNo],
    );
    ...
    const [
      ...
      connectionStatusCode
        ? client.query<{ connection_status_id: string }>(
            "SELECT connection_status_id FROM mt_connection_status WHERE status_code = $1 AND is_active = TRUE LIMIT 1",
            [connectionStatusCode],
          )
        : client.query<{ connection_status_id: string }>(
            "SELECT connection_status_id FROM mt_connection_status WHERE connection_status_id = $1 AND is_active = TRUE LIMIT 1",
            [current.rows[0].connection_status_id],
          ),
      serviceTypeCode
    ...
    if (serviceTypeCode && !serviceType.rows[0])
      throw new Error("INVALID_SERVICE_TYPE");
    if (routeCode && !route.rows[0]) throw new Error("INVALID_ROUTE");
    await client.query(
      `UPDATE service_accounts SET classification_id=$1, connection_type_id=$2, connection_status_id=$3, service_type_id=$4, route_id=$5, date_connected=$6::date, address=$7, updated_by=$8, updated_at=NOW() WHERE service_account_id=$9`,
      [
        classification.rows[0].classification_id,
        connectionType.rows[0].connection_type_id,
        connectionStatus.rows[0].connection_status_id,
    ...
    const failures: Record<string, [string, number]> = {
      NOT_FOUND: ["Service account not found.", 404],
      INVALID_CLASSIFICATION: [
        "The selected classification is unavailable.",
        400,
      ],
      INVALID_CONNECTION_TYPE: [
        "The selected connection type is unavailable.",
        400,
      ],
      INVALID_CONNECTION_STATUS: [
        "The selected connection status is unavailable.",
        400,
      ],
      INVALID_SERVICE_TYPE: ["The selected service type is unavailable.", 400],
      INVALID_ROUTE: ["The selected reading route is unavailable.", 400],
    };
    if (failures[code])
      return Response.json(
        { success: false, message: failures[code][0] },
        { status: failures[code][1] },
      );
