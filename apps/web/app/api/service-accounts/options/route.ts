...
export async function GET() {
  ...
  try {
    const [
      ...
      db.query(
        `SELECT service_type_code AS code, service_type_name AS name FROM mt_service_type WHERE is_active = TRUE ORDER BY service_type_name`,
      ),
      db.query(
        `SELECT route_code AS code, route_name AS name FROM mt_reading_route WHERE is_active = TRUE ORDER BY sequence_no NULLS LAST, route_code`,
      ),
    ]);
    return Response.json({
      ...
      data: {
        ...
        readingRoutes: readingRoutes.rows,
      },
    });
  } catch (error) {
