...
export async function POST(request: Request) {
  try {
    ...
    if (!sessionToken && !resetToken)
      return Response.json(
        { message: "Your password-change session has expired. Start again." },
        { status: 401 },
      );
    ...
    try {
      ...
      const resetResult =
        !sessionResult.rows[0] && resetToken
          ? await client.query<{ reset_id: string; user_id: string }>(
              "SELECT reset_id, user_id FROM password_reset_codes WHERE token_hash = $1 AND verified_at IS NOT NULL AND consumed_at IS NULL AND expires_at > NOW()",
              [hashSessionToken(resetToken)],
            )
          : { rows: [] as { reset_id: string; user_id: string }[] };
      const userId =
        sessionResult.rows[0]?.user_id ?? resetResult.rows[0]?.user_id;
      if (!userId)
        return Response.json(
          { message: "Your password-change session has expired. Start again." },
          { status: 401 },
      ...
      await client.query(
        "UPDATE users SET password_hash = $1, must_change_password = false, temporary_password_expires_at = NULL, updated_at = NOW() WHERE user_id = $2",
