...
export async function POST(request: Request, context: Context) {
  ...
  if (
    ...
    return Response.json(
      { success: false, message: "Unsupported application action." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (
      !allowedWorkflowActions(
        current.rows[0].status_code,
        current.rows[0].status_name,
      ).includes(action)
    )
      throw new Error("INVALID_TRANSITION");
    if (action === "complete") {
      const serviceAccount = await client.query(
        "SELECT service_account_id FROM service_accounts WHERE application_id = $1 LIMIT 1",
    ...
    await client.query(
      ...
      [
        ...
        applicationNo,
        JSON.stringify({
          statusCode: current.rows[0].status_code,
          status: current.rows[0].status_name,
        }),
        JSON.stringify({
    ...
    return Response.json({
      success: true,
      message: `Application ${applicationNo} was moved to ${target.status_name}.`,
    });
  } catch (error) {
      ...
      return Response.json(
        {
          success: false,
          message:
            "That status transition is not allowed for this application.",
        },
        { status: 409 },
      );
    if (code === "SERVICE_ACCOUNT_REQUIRED")
      return Response.json(
        {
          success: false,
          message:
            "Create the service account before completing this application.",
        },
        { status: 409 },
      );
    if (code === "STATUS_NOT_CONFIGURED")
      return Response.json(
        {
          success: false,
          message: "The required workflow status is not configured.",
        },
        { status: 409 },
      );
    console.error("Unable to change application status:", error);
    return Response.json(
      { success: false, message: "Unable to update the application status." },
      { status: 500 },
    );
  } finally {
    client.release();
