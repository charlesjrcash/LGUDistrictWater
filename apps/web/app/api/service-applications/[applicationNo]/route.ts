...
export async function GET(_request: Request, context: Context) {
  ...
  try {
    ...
    return Response.json({
      success: true,
      data: { ...detail.rows[0], statuses: statuses.rows },
    });
  } catch (error) {
    console.error("Unable to load service application:", error);
    return Response.json(
      { success: false, message: "Unable to load the service application." },
      { status: 500 },
    );
  }
}
...
export async function PUT(request: Request, context: Context) {
  ...
  try {
    ...
    const status =
      `${current.rows[0].status_code} ${current.rows[0].status_name}`.toUpperCase();
    if (!/(PENDING|SUBMIT|NEW)/.test(status))
      throw Object.assign(new Error("NOT_EDITABLE"), { status: 409 });
    ...
    return Response.json({
      success: true,
      message: `Application ${applicationNo} was updated successfully.`,
    });
  } catch (error) {
    ...
    if (code === "NOT_EDITABLE")
      return Response.json(
        { success: false, message: "Only pending applications can be edited." },
        { status: 409 },
      );
    ...
    return Response.json(
      { success: false, message: "Unable to update the service application." },
      { status: 500 },
    );
  } finally {
    client.release();
