...
} from "@/modules/service-applications/server";
...
export async function GET(request: Request) {
  ...
  if (date && !isIsoDate(date)) {
    return Response.json(
      { success: false, message: "Invalid application date." },
      { status: 400 },
    );
  }
  ...
  if (search) {
    values.push(`%${search}%`);
    where.push(
      `(sa.application_no ILIKE $${values.length} OR c.customer_name ILIKE $${values.length} OR c.customer_no ILIKE $${values.length})`,
    );
  }
  if (status) {
    values.push(status);
  ...
  try {
    ...
    return Response.json({
      ...
      summary: Object.fromEntries(
        Object.entries(summaryResult.rows[0]).map(([key, value]) => [
          key,
          Number(value),
        ]),
      ),
    ...
    return Response.json(
      { success: false, message: "Unable to load service applications." },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [customer, type, connectionType, meterSize, initialStatus] =
      await Promise.all([
        client.query<{ customer_id: string }>(
          "SELECT customer_id FROM customers WHERE customer_no = $1 LIMIT 1",
          [customerNo],
        ),
        ...
        requestedMeterSizeCode
          ? client.query<{ meter_size_id: string }>(
              "SELECT meter_size_id FROM mt_meter_size WHERE meter_size = $1 AND is_active = TRUE LIMIT 1",
              [requestedMeterSizeCode],
            )
    ...
    if (!customer.rows[0])
      throw Object.assign(new Error("CUSTOMER_NOT_FOUND"), { status: 400 });
    if (!type.rows[0])
      throw Object.assign(new Error("TYPE_NOT_FOUND"), { status: 400 });
    if (connectionTypeCode && !connectionType.rows[0])
      throw Object.assign(new Error("CONNECTION_TYPE_NOT_FOUND"), {
        status: 400,
      });
    if (requestedMeterSizeCode && !meterSize.rows[0])
      throw Object.assign(new Error("METER_SIZE_NOT_FOUND"), { status: 400 });
    if (!initialStatus)
      throw Object.assign(new Error("INITIAL_STATUS_NOT_FOUND"), {
        status: 409,
      });
    ...
    await client.query(
      ...
      [
        ...
        type.rows[0].application_type_id,
        initialStatus.application_status_id,
        applicationDate,
        connectionType.rows[0]?.connection_type_id ?? null,
        meterSize.rows[0]?.meter_size_id ?? null,
        remarks,
        auth.user.userId,
      ],
    );
    ...
    return Response.json(
      {
        success: true,
        data: { applicationNo },
        message: `Service application ${applicationNo} was created successfully.`,
      },
      { status: 201 },
    );
  } catch (error) {
    ...
    if (code === "CUSTOMER_NOT_FOUND")
      return Response.json(
        { success: false, message: "The selected customer no longer exists." },
        { status: 400 },
      );
    if (code === "TYPE_NOT_FOUND")
      return Response.json(
        {
          success: false,
          message: "The selected application type is unavailable.",
        },
        { status: 400 },
      );
    if (code === "CONNECTION_TYPE_NOT_FOUND")
      return Response.json(
        {
          success: false,
    ...
    if (code === "METER_SIZE_NOT_FOUND")
      return Response.json(
        { success: false, message: "The selected meter size is unavailable." },
        { status: 400 },
      );
      ...
      return Response.json(
        {
          success: false,
          message:
            "A Pending application status must be configured before creating applications.",
        },
        { status: 409 },
      );
    if (
      ...
      return Response.json(
        { success: false, message: "That application number already exists." },
        { status: 409 },
      );
    console.error("Unable to create service application:", error);
    return Response.json(
      { success: false, message: "Unable to save the service application." },
      { status: 500 },
    );
  } finally {
    client.release();
