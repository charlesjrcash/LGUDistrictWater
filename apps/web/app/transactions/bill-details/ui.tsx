...
type Detail = {
  ...
  billStatus?: string;
  rateId?: string | null;
  minimumCubicMeter?: string | null;
  maximumCubicMeter?: string | null;
...
function useData<T>(url: string) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Unable to load bill details.");
    } finally {
      setLoading(false);
    }
  }, [url]);
...
export function BillDetailsPage() {
  const [search, setSearch] = useState(""),
    [chargeType, setChargeType] = useState("");
  const query = useMemo(() => {
    const p = new URLSearchParams();
    if (search) p.set("search", search);
    if (chargeType) p.set("chargeType", chargeType);
    return p;
  }, [search, chargeType]);
  const { data, error, loading, load } = useData<Detail[]>(
    `/api/bill-details?${query}`,
  );
  return (
    <TransactionShell>
      <div className={styles.headingRow}>
        <div>
          ...
          <p className={styles.subtitle}>
            Review the charge lines that explain each bill calculation.
          </p>
        </div>
                  ...
                  <tr key={item.billDetailId}>
                    ...
                    <td>
                      <Link
                        ...
                        href={`/transactions/bill-details/${item.billDetailId}`}
                      >
                        View
                      </Link>
                    </td>
        ...
        <div>
          ...
          <Card
            ...
            rows={[
              ...
              ["Sequence No.", String(data.sequenceNo ?? "—")],
            ]}
          />
          <Card
            ...
            rows={[
              ["Rate ID", data.rateId || "—"],
              ["Minimum Cubic Meter", data.minimumCubicMeter || "—"],
              ["Maximum Cubic Meter", data.maximumCubicMeter || "Open-ended"],
              ["Rate Type", data.rateType || "—"],
              ["Effective Date", date(data.effectiveDate)],
              ["Expiration Date", date(data.expirationDate)],
            ]}
          />
        </div>
      </div>
...
function Failure({ message, retry }: { message: string; retry: () => void }) {
  return (
    <div className={`${styles.panel} ${styles.errorState}`}>
      <h2>Bill details unavailable</h2>
      <p>{message}</p>
