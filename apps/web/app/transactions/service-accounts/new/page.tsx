...
export default async function Page({ searchParams }: { searchParams: Promise<{ application?: string }> }) {
  ...
  if (!applicationNo) return <ModuleShell active="service-accounts"><div className={`${styles.panel} ${styles.empty}`}><h2>Approved application required</h2><p>Create service accounts from an approved Service Application so customer and application information remain linked.</p><Link href="/transactions/service-applications" className={styles.button}>View Service Applications</Link></div></ModuleShell>;
