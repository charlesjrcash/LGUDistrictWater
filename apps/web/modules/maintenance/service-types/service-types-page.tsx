...
export default function ServiceTypesPage() {
  ...
  const [editingServiceTypeId, setEditingServiceTypeId] = useState<
    string | null
  >(null);
  ...
  async function loadServiceTypes() {
    try {
      ...
      if (!response.ok || !result.success)
        throw new Error(result.message || "Failed to load service types.");
  ...
  const filteredServiceTypes = useMemo(() => {
    ...
    return serviceTypes.filter((serviceType) => {
      const matchesSearch =
        !value ||
        serviceType.service_type_code.toLowerCase().includes(value) ||
        serviceType.service_type_name.toLowerCase().includes(value) ||
        (serviceType.description ?? "").toLowerCase().includes(value);
      const matchesStatus =
        statusFilter === "all" ||
  ...
  }, [serviceTypes, search, statusFilter]);
  ...
  function updateForm(field: keyof ServiceTypeForm, value: string | boolean) {
    setForm((current) => ({ ...current, [field]: value }));
  }
  function handleEditServiceType(serviceType: ServiceType) {
    setEditingServiceTypeId(serviceType.service_type_id);
    setForm({
  ...
  async function handleSaveServiceType(
    ...
    setSaving(true);
    try {
      const isEditing = editingServiceTypeId !== null;
      const response = await fetch("/api/service-types", {
        method: isEditing ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...(isEditing ? { service_type_id: editingServiceTypeId } : {}),
          service_type_code: form.service_type_code.trim(),
          service_type_name: form.service_type_name.trim(),
      ...
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(result.message || "Unable to save service type.");
      ...
      setFormError(
        error instanceof Error ? error.message : "Unable to save service type.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage service types used by service accounts.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                  <tr>
                    <th className="px-6 py-4 font-medium">Service Type Code</th>
                    <th className="px-6 py-4 font-medium">Service Type Name</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
            <div className="border-b border-gray-200 px-6 py-5">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Define a service type and its availability.
              </p>
            </div>
            <form onSubmit={handleSaveServiceType}>
              <div className="space-y-5 px-6 py-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <FormInput
                    ...
                    onChange={(value) => updateForm("service_type_code", value)}
                    placeholder="e.g. RES"
                    required
                  />
                  <FormInput
                    ...
                    onChange={(value) => updateForm("service_type_name", value)}
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 px-6 py-4">
                ...
                >
                  {saving ? "Saving..." : "Save Service Type"}
                </button>
              </div>
            </form>
          </div>
