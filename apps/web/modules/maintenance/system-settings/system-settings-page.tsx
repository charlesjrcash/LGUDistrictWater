...
type SystemSetting = {
  ...
  data_type: "TEXT" | "INTEGER" | "BOOLEAN";
...
export default function SystemSettingsPage() {
  ...
  async function loadSettings() {
    try {
      setLoading(true);
      setError("");
      const response = await fetch("/api/system-settings?includeInactive=true");
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(result.message || "Failed to load system settings.");
      setSettings(result.data);
    } catch (caught) {
      console.error("System settings loading failed:", caught);
      setError(
  ...
  function openEdit(setting: SystemSetting) {
    ...
    setValue(
      setting.data_type === "BOOLEAN"
        ? setting.setting_value?.trim().toLowerCase() === "true"
          ? "true"
          : "false"
        : (setting.setting_value ?? ""),
    );
    setActive(setting.is_active);
    setError("");
  }
  function closeModal() {
    if (!saving) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const settingValue =
        editing.data_type === "BOOLEAN"
          ? value === "true"
            ...
            : "FALSE"
          : value.trim();
      if (editing.data_type === "INTEGER" && !/^[+-]?\d+$/.test(settingValue)) {
        throw new Error("Please enter a valid integer value.");
      }
      const response = await fetch("/api/system-settings", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          setting_id: editing.setting_id,
      ...
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(result.message || "Unable to update system setting.");
      await loadSettings();
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Manage centralized configuration used by the water billing system.
          </p>
        </div>
        {success && (
          <div className="mb-5 flex justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            <span>{success}</span>
            <button
              type="button"
              onClick={() => setSuccess("")}
                  ...
                  {settings.map((setting) => (
                    <tr key={setting.setting_id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-slate-700">
                        {setting.data_type === "BOOLEAN"
                          ? setting.setting_value?.toUpperCase() === "TRUE"
                            ? "True"
                            : "False"
                          : (setting.setting_value ?? "—")}
                      </td>
          ...
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            <div className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {editing.setting_key} · {editing.data_type}
                </p>
              </div>
              <button
                type="button"
                ...
                disabled={saving}
                aria-label="Close"
                className="-mr-2 -mt-1 rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600"
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
              ...
              <div className="space-y-5 px-6 py-6">
                {editing.data_type === "BOOLEAN" ? (
                  ...
                  <div>
                    ...
                    <input
                      type={editing.data_type === "INTEGER" ? "number" : "text"}
                      step={editing.data_type === "INTEGER" ? "1" : undefined}
                      value={value}
