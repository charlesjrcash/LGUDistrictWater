...
export default function PuroksPage() {
  ...
  const [form, setForm] = useState<PurokForm>(emptyForm);
  ...
  async function loadData() {
    try {
      ...
      const [pr, br] = await Promise.all([p.json(), b.json()]);
      if (!p.ok || !pr.success)
        throw new Error(pr.message || "Failed to load puroks.");
      if (!b.ok || !br.success)
        throw new Error(br.message || "Failed to load barangays.");
      setPuroks(pr.data);
      setBarangays(br.data);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to load puroks.");
    } finally {
      setLoading(false);
    }
  }
    ...
    return puroks.filter(
      (p) =>
        (!q ||
          ...
          p.purok_name.toLowerCase().includes(q) ||
          (p.purok_code ?? "").toLowerCase().includes(q) ||
          (p.description ?? "").toLowerCase().includes(q)) &&
        (statusFilter === "all" ||
          (statusFilter === "active" && p.is_active) ||
          (statusFilter === "inactive" && !p.is_active)),
    );
  }, [puroks, search, statusFilter]);
  function update(field: keyof PurokForm, value: string | boolean) {
  ...
  function edit(p: Purok) {
    setEditingPurokId(p.purok_id);
    setForm({
      barangay_id: p.barangay_id,
      purok_code: p.purok_code ?? "",
  ...
  async function save(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const editing = editingPurokId !== null;
      const r = await fetch("/api/puroks", {
        method: editing ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...(editing ? { purok_id: editingPurokId } : {}),
          barangay_id: form.barangay_id,
          purok_code: form.purok_code.trim() || null,
          purok_name: form.purok_name.trim(),
      ...
      if (!r.ok || !data.success)
        throw new Error(data.message || "Unable to save purok.");
      await loadData();
      setForm({ ...emptyForm });
      setEditingPurokId(null);
      setShowModal(false);
    } catch (e) {
      setFormError(e instanceof Error ? e.message : "Unable to save purok.");
    } finally {
      setSaving(false);
    }
  }
  const activeBarangays = barangays.filter(
    ...
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-gray-900">Puroks</h1>
            <p className="mt-1 text-sm text-gray-500">
              Manage puroks within each barangay.
            </p>
          </div>
          <button
            type="button"
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium">Purok Code</th>
                    <th className="px-6 py-4 font-medium">Purok Name</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((p) => (
                    <tr key={p.purok_id} className="hover:bg-gray-50">
            ...
            <div className="flex items-start justify-between border-b border-gray-200 px-6 py-5">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">
                  {editingPurokId ? "Edit Purok" : "Add Purok"}
                </h2>
                <p className="mt-1 text-sm text-gray-500">
                  Define a purok under a barangay.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={save}>
              <div className="space-y-5 px-6 py-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <Field
                    label="Purok Code"
                    value={form.purok_code}
                    onChange={(v) => update("purok_code", v)}
                  />
                  <Field
                    label="Purok Name"
                    value={form.purok_name}
                    onChange={(v) => update("purok_name", v)}
                    required
                  />
                </div>
                <div>
