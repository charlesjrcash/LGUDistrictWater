...
export default function UnitsPage() {
  ...
  async function loadUnits() {
    try {
      ...
      console.error("Units loading failed:", caught);
      setError(
  ...
  const filteredUnits = useMemo(() => {
    ...
    return units.filter((unit) => {
      const matchingText =
        !value ||
        unit.unit_code.toLowerCase().includes(value) ||
        unit.unit_name.toLowerCase().includes(value) ||
        (unit.description ?? "").toLowerCase().includes(value);
  ...
  function update(field: keyof UnitForm, value: string | boolean) {
    setForm((current) => ({ ...current, [field]: value }));
  }
  async function save(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setFormError("");
    if (!form.unit_code.trim()) return setFormError("Unit Code is required.");
    if (!form.unit_name.trim()) return setFormError("Unit Name is required.");
    setSaving(true);
    try {
      ...
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(result.message || "Unable to save unit of measure.");
      await loadUnits();
      ...
      console.error("Unit save failed:", caught);
      setFormError(
        caught instanceof Error
          ? caught.message
          : "Unable to save unit of measure.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage units of measure used by the system.
            </p>
          </div>
          <button
            type="button"
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
            ...
            <form onSubmit={save}>
              <div className="space-y-5 px-6 py-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <Input
                    ...
                    onChange={(value) => update("unit_code", value)}
                    maxLength={20}
                    placeholder="e.g. PCS"
                  />
                  <Input
                    ...
                    onChange={(value) => update("unit_name", value)}
                    maxLength={50}
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 px-6 py-4">
                ...
                >
                  {saving ? "Saving..." : "Save Unit of Measure"}
                </button>
              </div>
            </form>
          </div>
