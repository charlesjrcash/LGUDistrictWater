...
export default function ConnectionTypesPage() {
  ...
  const [editingConnectionTypeId, setEditingConnectionTypeId] = useState<
    string | null
  >(null);
  ...
  const filteredConnectionTypes = useMemo(() => {
    ...
    return connectionTypes.filter((connectionType) => {
      const matchesSearch =
        !searchValue ||
        connectionType.connection_type_code
          .toLowerCase()
  ...
  function updateForm(
    field: keyof ConnectionTypeForm,
  ...
  async function handleSaveConnectionType(
    ...
    try {
      ...
      setFormError(
        error instanceof Error
          ? error.message
          : "Unable to save connection type.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage connection types used by the water service application.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredConnectionTypes.map((connectionType) => (
                    <tr
                      key={connectionType.connection_type_id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          type="button"
                          onClick={() =>
                            handleEditConnectionType(connectionType)
                          }
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
            <div className="border-b border-gray-200 px-6 py-5">
              <h2 className="text-xl font-semibold text-gray-900">
                {editingConnectionTypeId
                  ? "Edit Connection Type"
                  : "Add Connection Type"}
              </h2>
              <p className="mt-1 text-sm text-gray-500">
                Define a connection type and its availability.
              </p>
            </div>
            <form onSubmit={handleSaveConnectionType}>
              <div className="space-y-5 px-6 py-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <FormInput
                    ...
                    value={form.connection_type_code}
                    onChange={(value) =>
                      updateForm("connection_type_code", value)
                    }
                    placeholder="e.g. NEW"
                    required
                  />
                  <FormInput
                    ...
                    value={form.connection_type_name}
                    onChange={(value) =>
                      updateForm("connection_type_name", value)
                    }
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 px-6 py-4">
                ...
                >
                  {saving ? "Saving..." : "Save Connection Type"}
                </button>
              </div>
            </form>
          </div>
