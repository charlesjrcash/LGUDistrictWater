...
export default function DisconnectionReasonsPage() {
  ...
  const [form, setForm] = useState<ReasonForm>(emptyForm);
  ...
  async function loadReasons() {
    try {
      setLoading(true);
      setError("");
      const response = await fetch("/api/disconnection-reason");
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(
          result.message || "Failed to load disconnection reasons.",
        );
      setReasons(result.data);
    } catch (caught) {
      console.error("Disconnection reasons loading failed:", caught);
      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to load disconnection reasons.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredReasons = useMemo(() => {
    ...
    return reasons.filter((reason) => {
      const matchingText =
        !value ||
        reason.reason_code.toLowerCase().includes(value) ||
        reason.reason_name.toLowerCase().includes(value) ||
        (reason.description ?? "").toLowerCase().includes(value);
      const matchingStatus =
        statusFilter === "all" ||
        (statusFilter === "active" && reason.is_active) ||
        (statusFilter === "inactive" && !reason.is_active);
      return matchingText && matchingStatus;
    });
  }, [reasons, search, statusFilter]);
  function openAdd() {
    setEditingId(null);
    setForm({ ...emptyForm });
    setFormError("");
  ...
  function edit(reason: Reason) {
    setEditingId(reason.reason_id);
    setForm({
  ...
  function update(field: keyof ReasonForm, value: string | boolean) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.reason_code.trim())
      return setFormError("Reason Code is required.");
    if (!form.reason_name.trim())
      return setFormError("Reason Name is required.");
    setSaving(true);
    try {
      ...
      if (!response.ok || !result.success)
        throw new Error(
          result.message || "Unable to save disconnection reason.",
        );
      await loadReasons();
      ...
      console.error("Disconnection reason save failed:", caught);
      setFormError(
        caught instanceof Error
          ? caught.message
          : "Unable to save disconnection reason.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage reasons used when a customer connection is disconnected.
            </p>
          </div>
          <button
            type="button"
          ...
          <div className="border-b border-gray-200 p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <input
                ...
                placeholder="Search disconnection reasons..."
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium">Reason Name</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredReasons.map((reason) => (
                    <tr key={reason.reason_id} className="hover:bg-gray-50">
                  ...
                  {filteredReasons.length === 0 && (
                    <tr>
                      ...
                      >
                        No disconnection reasons found.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
            ...
            <form onSubmit={save}>
              <div className="space-y-5 px-6 py-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <Input
                    ...
                    onChange={(value) => update("reason_code", value)}
                    maxLength={30}
                    placeholder="e.g. DISC-001"
                  />
                  <Input
                    label="Reason Name"
                    required
                    value={form.reason_name}
                    onChange={(value) => update("reason_name", value)}
                    maxLength={150}
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 px-6 py-4">
                ...
                >
                  {saving ? "Saving..." : "Save Disconnection Reason"}
                </button>
              </div>
            </form>
          </div>
