...
type M = { module_id: string; module_code: string; module_name: string };
type F = {
  permission_code: string;
  permission_name: string;
  module_id: string;
...
export default function PermissionsPage() {
  ...
  useEffect(() => {
    void Promise.all([load(), loadModules()]);
  }, []);
  async function load() {
    try {
      setLoading(true);
  ...
  async function loadModules() {
    const r = await fetch("/api/system-modules"),
      x = await r.json();
    if (r.ok && x.success) setModules(x.data);
  }
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (p) =>
        (!v ||
          p.permission_code.toLowerCase().includes(v) ||
          p.permission_name.toLowerCase().includes(v) ||
          p.module_name.toLowerCase().includes(v)) &&
        (status === "all" ||
  ...
  async function save(e: React.FormEvent) {
    e.preventDefault();
    if (
      !form.permission_code.trim() ||
      !form.permission_name.trim() ||
      !form.module_id
    )
      return setFormError("Please select a valid active module.");
    setSaving(true);
    try {
      const r = await fetch("/api/permissions", {
          method: id ? "PUT" : "POST",
  ...
  async function toggle(p: P) {
    if (
      !window.confirm(
        `Are you sure you want to ${p.is_active ? "deactivate" : "activate"} this permission?`,
      )
    )
      return;
    const r = await fetch("/api/permissions", {
    ...
    if (r.ok && x.success) {
      ...
      setError(
        x.message || "The permission could not be saved. Please try again.",
      );
  }
  return (
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage system permissions and the actions available within each
              module.
            </p>
          </div>
          <button
        ...
        <div className="overflow-hidden rounded-xl border bg-white">
          <div className="flex gap-3 border-b border-gray-200 p-5">
            <input
              ...
              className="w-full max-w-md rounded-lg border px-4 py-2.5 text-sm"
            />
            <select
              value={status}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                  <tr>
                    <th className="px-6 py-4">Permission Code</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filtered.map((p) => (
                    <tr key={p.permission_id}>
                      ...
                      <td className="px-6 py-4">{p.permission_name}</td>
                      <td className="px-6 py-4">
                        {p.module_code} - {p.module_name}
                      </td>
