...
const emptyForm: ReceiptTypeForm = {
  receipt_type_code: "",
  receipt_type_name: "",
  description: "",
  is_active: true,
...
export default function ReceiptTypesPage() {
  ...
  const [form, setForm] = useState<ReceiptTypeForm>(emptyForm);
  ...
  async function loadTypes() {
    try {
      setLoading(true);
      setError("");
      const response = await fetch("/api/receipt-type");
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(result.message || "Failed to load receipt types.");
      setTypes(result.data);
    } catch (caught) {
      console.error("Receipt types loading failed:", caught);
      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to load receipt types.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredTypes = useMemo(() => {
    ...
    return types.filter((type) => {
      const matchingText =
        !value ||
        type.receipt_type_code.toLowerCase().includes(value) ||
        type.receipt_type_name.toLowerCase().includes(value) ||
  ...
  function edit(type: ReceiptType) {
    setEditingId(type.receipt_type_id);
    setForm({
      receipt_type_code: type.receipt_type_code,
      receipt_type_name: type.receipt_type_name,
  ...
  function update(field: keyof ReceiptTypeForm, value: string | boolean) {
    setForm((current) => ({ ...current, [field]: value }));
  }
  async function save(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setFormError("");
    if (!form.receipt_type_code.trim())
      return setFormError("Receipt Type Code is required.");
    if (!form.receipt_type_name.trim())
      return setFormError("Receipt Type Name is required.");
    setSaving(true);
    try {
      ...
      const response = await fetch("/api/receipt-type", {
        ...
        body: JSON.stringify({
          ...(editing ? { receipt_type_id: editingId } : {}),
          receipt_type_code: form.receipt_type_code.trim(),
          receipt_type_name: form.receipt_type_name.trim(),
      ...
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(result.message || "Unable to save receipt type.");
      await loadTypes();
      ...
      console.error("Receipt type save failed:", caught);
      setFormError(
        caught instanceof Error
          ? caught.message
          : "Unable to save receipt type.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage the receipt types available for payment transactions.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                  <tr>
                    <th className="px-6 py-4 font-medium">Receipt Type Code</th>
                    <th className="px-6 py-4 font-medium">Receipt Type Name</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredTypes.map((type) => (
                    <tr key={type.receipt_type_id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 font-medium text-gray-900">
                        {type.receipt_type_code}
                      </td>
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
            <div className="flex items-start justify-between border-b border-gray-200 px-6 py-5">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">
                  {editingId ? "Edit Receipt Type" : "Add Receipt Type"}
                </h2>
                <p className="mt-1 text-sm text-gray-500">
            ...
            <form onSubmit={save}>
              <div className="space-y-5 px-6 py-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <Input
                    ...
                    required
                    value={form.receipt_type_code}
                    onChange={(value) => update("receipt_type_code", value)}
                    maxLength={30}
                    placeholder="e.g. OR"
                  />
                  <Input
                    ...
                    required
                    value={form.receipt_type_name}
                    onChange={(value) => update("receipt_type_name", value)}
                    maxLength={100}
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 px-6 py-4">
                ...
                >
                  {saving ? "Saving..." : "Save Receipt Type"}
                </button>
              </div>
            </form>
          </div>
