...
export function ApplicationForm({ applicationNo, initialCustomerNo }: { applicationNo?: string; initialCustomerNo?: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/transactions/service-applications/${encodeURIComponent(destination)}?${editing ? "updated" : "created"}=1`);
    } catch (submitError) {
  ...
  return (
    <ModuleShell>
      <div className={styles.formShell}>
        <Link href={editing && applicationNo ? `/transactions/service-applications/${encodeURIComponent(applicationNo)}` : "/transactions/service-applications"} className={styles.backLink}>← {editing ? applicationNo : "Service Applications"}</Link>
        ...
        <form className={`${styles.panel} ${styles.formPanel}`} onSubmit={handleSubmit} noValidate>
          ...
          {loading ? <div className={styles.loading}><div className={styles.skeleton} style={{ height: 24, marginBottom: 18 }} /><div className={styles.skeleton} style={{ height: 130 }} /></div> : <>
            <section className={styles.section}>
              ...
              {!editing && !selectedCustomer && <div className={styles.fullField}><label className={styles.label} htmlFor="customer-search">Search customer <span className={styles.required}>*</span></label><div className={`${styles.searchWrap} ${styles.browseInput}`}><svg className={styles.searchIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></svg><input id="customer-search" className={styles.input} value={customerQuery} onFocus={() => { if (!customerQuery && !customerResults.length) void browseCustomers(); else setCustomerPickerOpen(true); }} onChange={(event) => { setCustomerQuery(event.target.value); setCustomerPickerOpen(true); }} placeholder="Search by customer name, number, or contact number..." autoComplete="off" /><button type="button" className={styles.browseButton} disabled={searching} onClick={() => void browseCustomers()}>{searching ? "Loading…" : "Browse Customers"}</button></div>{fieldErrors.customerNo && <div className={styles.fieldError}>{fieldErrors.customerNo}</div>}{customerPickerOpen && <div className={styles.customerResults}>{searching ? <div className={styles.loading} style={{ padding: 18 }}>Loading customers…</div> : customerResults.length ? customerResults.map((customer) => <button key={customer.customerNo} type="button" className={styles.customerResult} onClick={() => { setSelectedCustomer(customer); setCustomerResults([]); setCustomerQuery(""); setCustomerPickerOpen(false); }}><span><strong className={styles.strong}>{customer.name}</strong><span className={styles.muted}>{customer.customerNo} · {customer.barangay || customer.address || "Address not available"}</span></span><span className={styles.muted}>{customer.contactNo || "No contact"}</span></button>) : <div className={styles.loading} style={{ padding: 18 }}>{customerQuery.trim().length >= 2 ? "No customers match your search." : "No customers are available."}</div>}</div>}<p className={styles.muted}>Customer not listed? <Link href="/transactions/customers/new?returnTo=/transactions/service-applications/new" className={styles.viewLink}>＋ Add New Customer</Link></p></div>}
            ...
            <div className={styles.formActions}><Link href={editing && applicationNo ? `/transactions/service-applications/${encodeURIComponent(applicationNo)}` : "/transactions/service-applications"} className={styles.secondaryButton}>Cancel</Link><button className={styles.button} disabled={submitting}>{submitting ? "Saving…" : editing ? "Save Changes" : "Save Application"}</button></div>
          </>}
        </form>
      </div>
    </ModuleShell>
