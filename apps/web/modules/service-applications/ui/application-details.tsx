...
export function ApplicationDetails({ applicationNo }: { applicationNo: string }) {
  ...
  return <ModuleShell>
    ...
    <div className={styles.detailGrid}>
      <div>
        <section className={styles.detailCard}><div className={styles.cardHeading}><h2>Applicant Information</h2><Link href={`/customers/${encodeURIComponent(application.customer.customerNo)}`} className={styles.viewLink}>View Customer Profile →</Link></div><DetailItems items={applicantFields} /></section>
      ...
      <aside>
        ...
        <section className={styles.detailCard}><div className={styles.cardHeading}><h2>Application Actions</h2></div><div className={styles.actionStack}>{statusKind === "pending" && <><Link href={`/service-applications/${encodeURIComponent(application.applicationNo)}/edit`} className={styles.secondaryButton}>Edit Application</Link><button className={styles.dangerButton} onClick={() => setConfirmAction("reject")}>Reject</button><button className={styles.button} onClick={() => setConfirmAction("inspect")}>Send for Inspection</button></>}{statusKind === "processing" && <><button className={styles.dangerButton} onClick={() => setConfirmAction("reject")}>Reject</button><button className={styles.button} onClick={() => setConfirmAction("approve")}>Approve Application</button></>}{statusKind === "approved" && <><div className={styles.successNotice} style={{ margin: 0 }}>This application is approved and ready for the next step.</div>{application.serviceAccountControlNo ? <Link className={styles.button} href={`/service-accounts/${encodeURIComponent(application.serviceAccountControlNo)}`}>Service Account Created</Link> : <Link className={styles.button} href={`/service-accounts/new?application=${encodeURIComponent(application.applicationNo)}`}>Create Service Account</Link>}<button className={styles.secondaryButton} onClick={() => setConfirmAction("complete")}>Complete Application</button></>}{statusKind === "rejected" && <div className={styles.notice} style={{ margin: 0 }}>This application has been rejected. No further workflow actions are available.</div>}{!["pending", "processing", "approved", "rejected"].includes(statusKind) && <div className={styles.muted}>{application.serviceAccountControlNo ? "Service Account Created" : "No Service Account Yet"}</div>}</div></section>
      </aside>
    </div>
