...
export function AccountDetails({ controlNo }: { controlNo: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const response = await fetch(`/api/service-accounts/${encodeURIComponent(controlNo)}`, { cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.message); setAccount(body.data); }
    catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Unable to load the service account."); }
    finally { setLoading(false); }
  }, [controlNo]);
  useEffect(() => { const timer = window.setTimeout(() => { void load(); const params = new URLSearchParams(window.location.search); if (params.has("created")) setNotice(`Service account ${controlNo} was created successfully.`); if (params.has("updated")) setNotice(`Service account ${controlNo} was updated successfully.`); if (params.has("created") || params.has("updated")) window.history.replaceState({}, "", window.location.pathname); }, 0); return () => window.clearTimeout(timer); }, [controlNo, load]);

  if (loading) return <ModuleShell active="service-accounts"><div className={`${styles.panel} ${styles.loading}`}><div className={styles.skeleton} style={{ height: 220 }} /></div></ModuleShell>;
  if (!account) return <ModuleShell active="service-accounts"><Link className={styles.backLink} href="/transactions/service-accounts">← Service Accounts</Link><div className={`${styles.panel} ${styles.errorState}`}><h2>Service account unavailable</h2><p>{error}</p><button className={styles.secondaryButton} onClick={load}>Try again</button></div></ModuleShell>;
  ...
  const accountFields = [["Control No.",account.controlNo],["Classification",account.classification],["Connection Type",account.connectionType],["Status",account.status],["Date Connected",formatDate(account.dateConnected,true)]];
  return <ModuleShell active="service-accounts">
    <Link className={styles.backLink} href="/transactions/service-accounts">← Service Accounts</Link>{notice && <div className={styles.successNotice}>{notice}</div>}{error && <div className={styles.notice}>{error}</div>}
    <header className={styles.detailHeader}><div><div className={styles.applicationNumber}><h1>{account.controlNo}</h1><AccountStatusBadge code={account.statusCode} name={account.status} /></div><p className={styles.detailCustomer}>{account.customer.name}<span>Customer No. {account.customer.customerNo}</span></p></div><Link className={styles.secondaryButton} href={`/transactions/service-accounts/${encodeURIComponent(account.controlNo)}/edit`}>Edit Service Account</Link></header>
    ...
    <div className={styles.detailGrid}><div>
      <section className={styles.detailCard}><div className={styles.cardHeading}><h2>Customer Information</h2><Link className={styles.viewLink} href={`/transactions/customers/${encodeURIComponent(account.customer.customerNo)}`}>View Customer Profile →</Link></div><div className={styles.detailItems}>{customerFields.map(([label,value]) => <div className={styles.detailItem} key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section>
      ...
      {account.application && <section className={styles.detailCard}><div className={styles.cardHeading}><h2>Application Reference</h2><Link className={styles.viewLink} href={`/transactions/service-applications/${encodeURIComponent(account.application.applicationNo)}`}>View Application →</Link></div><div className={styles.detailItems}>{[["Application No.",account.application.applicationNo],["Application Type",account.application.applicationType],["Application Date",formatDate(account.application.applicationDate,true)],["Application Status",account.application.status]].map(([label,value]) => <div className={styles.detailItem} key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section>}
