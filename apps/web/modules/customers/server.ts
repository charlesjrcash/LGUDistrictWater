...
export async function nextCustomerNumber(client: PoolClient, userId: string) {
  ...
  await client.query(
    "SELECT pg_advisory_xact_lock(hashtext('customers.customer_no'))",
  );
  const result = await client.query<{ next_number: string }>(
    `SELECT COALESCE(MAX((regexp_match(customer_no, '^CUS-([0-9]+)$'))[1]::bigint),0)+1 AS next_number FROM customers`,
  );
...
export function normalizeCustomerPayload(body: Record<string, unknown>) {
  ...
  return {
    customerName: value(body.customerName, 200),
    firstName: value(body.firstName, 100) || null,
    middleName: value(body.middleName, 100) || null,
    lastName: value(body.lastName, 100) || null,
    address: value(body.address, 1000) || null,
    barangayCode: value(body.barangayCode, 20) || null,
    purokCode: value(body.purokCode, 20) || null,
    contactNo: value(body.contactNo, 50) || null,
    email: value(body.email, 150).toLowerCase() || null,
    status: value(body.status, 30).toUpperCase() || "ACTIVE",
  };
}

export function validateCustomer(
  payload: ReturnType<typeof normalizeCustomerPayload>,
) {
  const errors: Record<string, string> = {};
  if (!payload.customerName) errors.customerName = "Enter the customer name.";
  if (payload.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.email))
    errors.email = "Enter a valid email address.";
  if (payload.contactNo && !/^[+\d][\d\s().-]{6,24}$/.test(payload.contactNo))
    errors.contactNo = "Enter a valid contact number.";
  if (!["ACTIVE", "INACTIVE"].includes(payload.status))
    errors.status = "Select a valid customer status.";
  return errors;
}
