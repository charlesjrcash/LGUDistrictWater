...
function OrganizationHeader({organization}:{organization?:Organization}){if(!organization||!Object.values(organization).some(Boolean))return null;const contacts=[["TIN",organization.tin],["VAT No.",organization.vatNo],["Contact",organization.contactNo],["Email",organization.email],["Website",organization.website]].filter(([,value])=>Boolean(value));return <header className={styles.organizationHeader}>{organization.logoPath&&<img className={styles.organizationLogo} src={organization.logoPath} alt=""/>}<div className={styles.organizationText}>{organization.name&&<strong className={styles.organizationName}>{organization.name}</strong>}{organization.officeName&&<span>{organization.officeName}</span>}{organization.address&&<span>{organization.address}</span>}{contacts.length>0&&<div className={styles.organizationContacts}>{contacts.map(([label,value])=><span key={label}>{label}: {value}</span>)}</div>}</div></header>}
function OrganizationFooterNote({note}:{note?:string|null}){return note?<p className={styles.organizationFooterNote}>{note}</p>:null}
